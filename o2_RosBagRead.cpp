...
Mat logTransform2(Mat srcImage, float c = 1)
{
    if (srcImage.empty())
        cout << "No data" << endl;

    Mat gray_float(srcImage.size(), CV_32FC1);  
    srcImage.convertTo(gray_float, CV_32FC1);
    Mat resultImage = Mat::zeros(srcImage.size(), srcImage.type());
    Mat log_image_32bit = Mat::zeros(srcImage.size(), CV_32FC1);
    float a = log(2.71828 + 8);
    double gray = 0;
    for (int i = 0; i < srcImage.rows; i++){
        for (int j = 0; j < srcImage.cols; j++){
            gray = (double)gray_float.at<float>(i, j) / 255;
            gray = c * log1p((double)(gray)) / a ;
            log_image_32bit.at<float>(i, j) = saturate_cast<float>(gray*255);
        }
    }
    cv::normalize(log_image_32bit, resultImage, 0, 255, NORM_MINMAX);
    cv::convertScaleAbs(resultImage, resultImage);
    return resultImage;
}
Mat logTransform1(Mat srcImage, float c = 1)
{
    if (srcImage.empty())
        cout << "No data" << endl;
    ...
    Mat gray_float(srcImage.size(), CV_32FC1);  
    // cv::cvConvertScale(srcImage, gray_float, 1.0 / 255, 0); 
    srcImage.convertTo(gray_float, CV_32FC1);

    Mat resultImage = Mat::zeros(srcImage.size(), srcImage.type());

    // // 将图像转换回8位无符号整数类型
    Mat log_image_8bit = Mat::zeros(srcImage.size(), CV_32FC1);
    ...
    for (int i = 0; i < srcImage.rows; i++)
    {
        ...
        {
            gray = (double)gray_float.at<float>(i, j) / 255;
            // cout << "gray_float  " << gray << endl;
            gray = c * log1p((double)(gray)) / a ;         // c * log(1+r) / log(v)
            // log_image_32bit.at<float>(i, j) = static_cast<float>(gray);
            log_image_8bit.at<float>(i, j) = saturate_cast<float>(gray*255);
        }
    }
    // //归一化处理
    cv::normalize(log_image_8bit, resultImage, 0, 255, NORM_MINMAX);
    cv::convertScaleAbs(resultImage, resultImage);
    // 显示结果图像
    return resultImage;
}

cv::Mat Logimage(const cv::Mat &src)
{
    ...
    for (int i = 0; i < src.rows; i++)
    {
        ...
        {
            // std::cout << src.at<cv::Vec1b>(i, j) << std::endl;
            dst.at<uchar>(i, j) = src.at<uchar>(i, j);
            // dst.at<uchar>(i, j) = log(1 + src.at<uchar>(i, j));
            // dst.at<cv::Vec3f>(i, j)[1] = log(1 + src.at<cv::Vec3b>(i, j)[1]);
            // dst.at<cv::Vec3f>(i, j)[2] = log(1 + src.at<cv::Vec3b>(i, j)[2]);
        }
    }
    //归一化到0~255
    normalize(dst, dst, 0, 255, CV_MMX);
    //转换成8bit图像显示
...
void test(std::vector<cv::Mat>& img) {
    ...
    if(imgpyr.size() >= 2)
    {
        ...
        for (int i = 0; i < pt2.size(); i++) {
            if (status[i]) {
                cv::circle(img2_CV, pt2[i], 2, cv::Scalar(0, 250, 0), 4);
        ...
        cv::imshow(" #0 left last - 1 left now# tracked by opencv", img2_CV);
    }
    if(imgpyr.size() >= 3)
    {
        ...
        for (int i = 0; i < pt3.size(); i++) {
            if (statu[i]) {
                cv::circle(img3_CV, pt3[i], 2, cv::Scalar(0, 250, 0), 4);
                cv::line(img3_CV, pt1[i], pt3[i], cv::Scalar(0, 250, 0),2);
            }
        }
        cv::resize(img3_CV,img3_CV,cv::Size(img3_CV.cols/show,img3_CV.rows/show));
        cv::imshow(" # 2 right last - 0 left last # tracked by opencv", img3_CV);

        Mat img1_CV;
        cv::cvtColor(img.at(0), img1_CV, cv::COLOR_GRAY2BGR);
        for (int i = 0; i < pt3.size(); i++) {
            if (statu[i]) {
                cv::circle(img1_CV, pt1[i], 2, cv::Scalar(0, 250, 0), 4);
                cv::line(img1_CV, pt3[i], pt1[i], cv::Scalar(0, 250, 0),2);
            }
        }
        cv::resize(img1_CV,img1_CV,cv::Size(img1_CV.cols/show,img1_CV.rows/show));
        cv::imshow("# 0 left last - 2 right last# tracked by opencv", img1_CV);

    }
    // std::cout << "4、 " << std::endl;    

    Mat img4_CV;
    cv::cvtColor(img.at(1), img4_CV, cv::COLOR_GRAY2BGR);
    for (int i = 0; i < pt1.size(); i++) {
        if (statu[i] || status[i]) {
            cv::circle(img4_CV, pt1[i], 2, cv::Scalar(0, 250, 0), 4);
            cv::line(img4_CV, pt3[i], pt1[i], cv::Scalar(150, 150, 0),2);
            cv::line(img4_CV, pt2[i], pt1[i], cv::Scalar(0, 250, 250),2);
        }
    }
    cv::resize(img4_CV,img4_CV,cv::Size(img4_CV.cols/show,img4_CV.rows/show));
    cv::imshow("stereo ", img4_CV);
    // std::cout << "5、 " << std::endl;
...
int main(int argc, char **argv) {
    ...
    std::string path_to_bag = "/home/tam/DataSet/20231116_CG170_baseball/1116_01_1699345333-fourth.bag"; 
    ...
    bool flag = false;
    static Mat img1; // left last
    static Mat img2; // right last 
    static Mat img3; // left curr
        ...
        if (iter->getTopic() == topic_cameras.at(0)) 
        {
            ...
            // {
                ...
                else 
                {
                    ...
                    cv::Mat tmp;
                    cv::resize(cv_ptr0->image, tmp, cv::Size(cv_ptr0->image.cols/2.0, cv_ptr0->image.rows/2.0));
                    img2 = cv_ptr0->image(cv::Rect(0, 0, cv_ptr0->image.cols, cv_ptr0->image.rows/2.0)); // left now 
                    cv::Mat img3_tmp = cv_ptr0->image(cv::Rect(0, cv_ptr0->image.rows/2.0, cv_ptr0->image.cols, cv_ptr0->image.rows/2.0)); // right last
                    // cv::Mat imgLeft(height/2, width, CV_8UC1, data);
                    // cv::Mat imgRight(height/2, width, CV_8UC1, data + width*height/2);
                    // message.images.push_back(im(cv::Rect(0, 0, im.cols, im.rows/2)));            // left
                    // message.images.push_back(im(cv::Rect(0, im.rows/2, im.cols, im.rows/2)));    // right
                    // cv::imshow("ads", tmp);
                    // cv::waitKey(0);
                    // if(!img1.empty())
                    //     cv::imshow("img1", img1);
                    ...
                    if(!img1.empty() && !img2.empty() && !img3.empty())
                    {
                        ...
                        // temp.push_back(img2);
                        // temp.push_back(img3);
                        temp.push_back(logTransform2(img1));    // left last
                        temp.push_back(logTransform2(img2));    // left now
                        temp.push_back(logTransform2(img3));    // right last
                        test(temp);
                    }
                    cv::waitKey(0);

                    img1 = img2.clone();    // left last
                    // img2 = cv_ptr0->image(cv::Rect(0, 0, cv_ptr0->image.cols, cv_ptr0->image.rows/2.0)); // right last
                    img3 = img3_tmp.clone(); //cv_ptr0->image(cv::Rect(0, cv_ptr0->image.rows/2.0, cv_ptr0->image.cols, cv_ptr0->image.rows/2.0)); // right last                    
                }
            // }
        }
