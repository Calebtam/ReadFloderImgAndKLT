...
class CameraFrustum {
public:
    void Create(const Camera& camera, double min_depth, double max_depth, double expansion_radius, const Sophus::SE3d& Twc) {
        ...
        // 计算视锥体的基础顶点
        const Eigen::Vector3d top_left = camera.UnprojectFromPixelCornerConv(Eigen::Vector2i(0, 0)).template cast<double>();
        const Eigen::Vector3d top_right = camera.UnprojectFromPixelCornerConv(Eigen::Vector2i(camera.width(), 0)).template cast<double>();
        const Eigen::Vector3d bottom_left = camera.UnprojectFromPixelCornerConv(Eigen::Vector2i(0, camera.height())).template cast<double>();
        const Eigen::Vector3d bottom_right = camera.UnprojectFromPixelCornerConv(Eigen::Vector2i(camera.width(), camera.height())).template cast<double>();
        ...
        // 计算视平面法线
        Eigen::Vector3d near_normal = (base_points[1] - base_points[0]).cross(base_points[2] - base_points[0]).normalized();
        Eigen::Vector3d far_normal = (base_points[5] - base_points[4]).cross(base_points[6] - base_points[4]).normalized();
