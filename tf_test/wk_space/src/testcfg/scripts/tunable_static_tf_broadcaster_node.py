...
from testcfg.cfg import TfConfig
...
class TunableStaticTFBroadcaster:
    def __init__(self):
        ...
        # print("===========================" + self.__static_transformStamped.header.frame_id)
        # print("===========================" + self.__static_transformStamped.child_frame_id)
        if self.__yaml_path != "":
            try:
                yaml_file = open(self.__yaml_path, "r+")
                yaml_data = yaml.safe_load(yaml_file)
                client = Client(rospy.get_name())
                data_ = yaml_data['dictitems']['groups']['dictitems']['param']
                client.update_configuration(data_)
                print( " === " + rospy.get_name())
                # config_data = dict(yaml_data)
                # print(config_data.tf_x)
                self.__set_tf(data_)
            except IOError:
