...
public:
  ...
  void compute_distort_jacobian(const Eigen::Vector2d &uv_norm, Eigen::MatrixXd &H_dz_dzn, Eigen::MatrixXd &H_dz_dzeta) override {
    ...
    // https://docs.openvins.com/update-feat.html#distortion-equi
    ...
    duv_dxy << cam_d(0), 0, 0, cam_d(1);        // [fx  0 ; 0  fy]

    // Jacobian of "normalized" pixel to normalized pixel
    // 雅可比矩阵，用于将归一化坐标映射到正常坐标
    ...
    // 总的雅可比矩阵，用于归一化像素坐标
    //  d （the distorted image coordinates on the image plane）
    //  d  (the normalized coordinates of the 3D feature)
    ...
    // d（the distorted image coordinates on the image plane）
    // d (distortion intrinsic parameters)
    // 观测对内参的雅可比
