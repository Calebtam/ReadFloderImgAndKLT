...
//  create_maker();//run only once
  ...
  double width_ = 0.1155 / 2; // x
  double height = 0.081 / 2; // y  
  std::vector<cv::Point3f> corners_3d_(4);
  // Mark 0 
  corners_3d_[0] = cv::Point3f(-width_, height, 0); // right down
  corners_3d_[1] = cv::Point3f(-(width_+tag.half_square_length), height, 0);  // left down
  corners_3d_[2] = cv::Point3f(-(width_+tag.half_square_length), height+tag.half_square_length, 0); // left up
  corners_3d_[3] = cv::Point3f(-width_, height+tag.half_square_length, 0); // right up
  corners_3d_my.push_back(corners_3d_);
  // Mark 1
  corners_3d_[1] = cv::Point3f(width_, height, 0); // l d
  corners_3d_[0] = cv::Point3f(width_+tag.half_square_length, height, 0);  // r d
  corners_3d_[3] = cv::Point3f(width_+tag.half_square_length, height+tag.half_square_length, 0); //r u
  corners_3d_[2] = cv::Point3f(width_, height+tag.half_square_length, 0); // l u
  corners_3d_my.push_back(corners_3d_);
  // Mark 2
  corners_3d_[3] = cv::Point3f(-width_, -height, 0); // r u
  corners_3d_[2] = cv::Point3f(-(width_+tag.half_square_length), -height, 0);  // l u
  corners_3d_[1] = cv::Point3f(-(width_+tag.half_square_length), -(height+tag.half_square_length), 0); // l d
  corners_3d_[0] = cv::Point3f(-width_, -(height+tag.half_square_length), 0);  // r d
  corners_3d_my.push_back(corners_3d_);
  // Mark 3
  corners_3d_[2] = cv::Point3f(width_, -height, 0); // l u
  corners_3d_[3] = cv::Point3f((width_+tag.half_square_length), -height, 0);  // r u
  corners_3d_[0] = cv::Point3f((width_+tag.half_square_length), -(height+tag.half_square_length), 0); // r d
  corners_3d_[1] = cv::Point3f(width_, -(height+tag.half_square_length), 0); // l d
  corners_3d_my.push_back(corners_3d_);
}
...
bool ArucoDetector::detect(const cv::Mat &image) {
  ...
  // for(auto idc:corners){
  //   // for(auto c:idc){
  //   for(int c = 0; c < idc.size(); c++){
  //     cv::circle(im_show, idc.at(c), 3, cv::Scalar(0,0+255*c/idc.size(),0), -1);
  //   }
  // }
...
bool ArucoDetector::get_pose(const cv::Mat &image, Eigen::Isometry3d& Tcw, double devAng){
  ...
  corners_3d_match.clear();
  for(auto id : ids_aruco){
    // std::cout << std::endl << id;
    // for(auto p:corners[id])
    std::cout << std::endl << id ;
    // if(id < 4)
    for(int i = 0; i < corners[id].size(); i++)
    {
      cv::Point2f uv = cam_intrinsic->undistort_cv(corners[id].at(i));
      p_2ds.push_back(uv);
      corners_3d_match.push_back(corners_3d_my.at(id).at(i));
      std::cout << "  |  " << corners[id].at(i)  << " " << corners_3d_my.at(id).at(i);
    }
  }
  std::cout << std::endl ;
  ...
  Eigen::MatrixXd cam_in = cam_intrinsic->get_value();
  std::cout << cam_in(0) << cam_in(1) << cam_in(2) << cam_in(3) << std::endl;


  // for(auto idc:corners){
    // for(auto c:idc){
    for(int c = 0; c < p_2ds.size(); c++){
      
      cv::Point2f tpt;
      tpt.x = p_2ds[c].x * cam_in(0) + cam_in(2);
      tpt.y = p_2ds[c].y * cam_in(1) + cam_in(3);
      cv::circle(im_show, tpt, 3, cv::Scalar(0, 255, 0), -1);
  ...
  // }
    ...
    for(int c = 0; c < p_2ds.size(); c++){
      ...
      cv::Point2f tpt = cam_intrinsic->distort_cv(p_2ds[c]);
      cv::circle(im_show, tpt, 3, cv::Scalar(0, 0, 255), -1);
    }
  cv::resize(im_show, im_show, im_show.size());
  cv::imshow("calibra result check ", im_show);
  cv::waitKey(10);

#endif

  // std::cout << std::endl << p_2ds.size() << "  " <<  corners_3d_match.size()<< std::endl;
  
  // calculate pose with pnp
  if(0)
  // if(p_2ds.size() == corners_3d_match.size())
  {

    // std::vector<cv::Vec3d> rvec, tvec;
    cv::Vec3d rvec, tvec;
    bool valid = cv::solvePnP(corners_3d_match, p_2ds, cv::Matx33d::eye(), cv::Mat(), rvec, tvec, false, cv::SOLVEPNP_IPPE);//0.116 ms in pc
    std::cout << "sol: r=" << rvec << "   t=" << tvec << "   valid=" << valid << std::endl;
...
//    printf("[TRAJ]: pnp: %.3f vs %.3f ms\n", (rT2 - rT1).total_microseconds() * 1e-3, (rT3 - rT2).total_microseconds() * 1e-3);
    ...
    else
    {
      ...
      for(size_t i = 0; i < p_2ds.size(); ++i){
        Eigen::Vector3d pw(corners_3d_match[i].x, corners_3d_match[i].y, corners_3d_match[i].z);
        ...
        if((pc2 - uv).norm() > 0.01){
          ...
          std::cout << RED << "pc est: " << pc2.transpose() << "                   ";
          std::cout << "pc mea: " << uv.transpose() << RESET << std::endl;
          pnp_valid = false;
          // break;
        }
      }
      PRINT_INFO("[aruco] pnp: solve the pnp \n");
