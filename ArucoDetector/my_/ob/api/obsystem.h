...
#include "cvAruco/aruco_detector.h"
...
public:
    ...
    void stationCallback(double ts, ChargestationInfo station);
    void SetOutData(Eigen::Isometry3d &Tra, ob_aruco_result &result);
...
private:  
    ...
    // thread
    std::mutex img_queue_mtx, station_mtx;
    std::deque<std::pair<double, cv::Mat>> img_queue;
    std::deque<std::pair<double, ChargestationInfo>> station_queue;
