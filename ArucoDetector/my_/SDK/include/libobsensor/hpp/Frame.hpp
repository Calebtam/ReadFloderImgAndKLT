...
/**
 * @brief Define the GPMFrame class, which inherits from the VideoFrame class
 */
class OB_EXTENSION_API GPMFrame : public VideoFrame {
public:
    explicit GPMFrame(Frame &frame);

    explicit GPMFrame(std::unique_ptr<FrameImpl> impl);

    ~GPMFrame() noexcept override = default;
};
...
template <typename T> bool Frame::is() {
    ...
    case OB_FRAME_GPM:
        return (typeid(T) == typeid(GPMFrame) || typeid(T) == typeid(VideoFrame));
    default:
